...
protocol PokemonServiciable {
    ...
    func getCombineList(limit: Int) -> AnyPublisher<PokemonList, Never>
}
...
struct NetworkService: HTTPClient, PokemonServiciable {
    ...
    func getCombineList(limit: Int) -> AnyPublisher<PokemonList, Never>{
        return sendReques(endpoint: PokemonEndpoint.pokemonListCombine(limit: limit), responseModel: PokemonList.self)
    }
